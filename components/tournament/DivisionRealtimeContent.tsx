...
export default function DivisionRealtimeContent({
  ...
  // 예선 연결 1라운드 TBD 슬롯에 조별 예상 배정 라벨 계산
  // (실제 진출 배정과 동일한 getPrelimSlotPlacements 매핑 사용)
  function getProjectedLabel(matchNumber: number, isP2: boolean): string | null {
    ...
    const placements = getPrelimSlotPlacements(sortedGroups.length, prelim.advancement_count ?? 2)
    ...
    const group = sortedGroups[placement.group]
